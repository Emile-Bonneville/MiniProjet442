...
err_t err, recv_err;
char messageReceived[TAILLE_MESSAGE_COUP_MAX];
...
len = netbuf_len(buf);
/* Vérification pour éviter un dépassement de tampon */
if (len < TAILLE_MESSAGE_COUP_MAX) {
netbuf_copy(buf, messageReceived, len);
messageReceived[len] = '\0'; /* Ajout du terminateur de chaîne */

/* Envoi du message décodé vers la file d'attente de la logique de jeu */
xQueueSend(QueueReceptionUDP, messageReceived, 0);
}
...
err_t err;
char messageToSend[TAILLE_MESSAGE_COUP_MAX];
static struct netconn *conn_emission;
static struct netbuf *buf;
static ip_addr_t addr_dest;

/* L'adresse IP du destinataire doit être adaptée à la configuration réseau */
IP4_ADDR(&addr_dest, 192, 168, 1, 90);

/* Attente de l'initialisation de LwIP */
osDelay(5000);
...
err = netconn_connect(conn_emission, &addr_dest, 8080);
if (err != ERR_OK) {
...
for(;;)
{
/* La tâche se bloque ici jusqu'à ce qu'un message soit présent dans la file */
if (xQueueReceive(QueueEmissionUDP, messageToSend, portMAX_DELAY) == pdPASS)
{
buf = netbuf_new();
netbuf_ref(buf, messageToSend, strlen(messageToSend));
